...
- 커다란 단일 코드 베이스를 더 작은 캡슐화된 기능 단위로 나누고 나눠진 작은 단위가 협력하 더 큰 단위로 동작하게 하는 것을 말한다
- 모듈은 자신을 사용하기 위한 API를 외부에 제공해야한다
- 자신이 구동되는데 필요한 환경을 명확히 나열할 수 있어야한다.
- 독립적으로 테스트될 수 있어야한다.
- 소프트웨어의 전체적 구조를 표현하기위해 관련된 클래스의 집합을 하나의 논리적인 단위로 묶는 구성요소를 모듈이라 하며 이를 자바에선 패키지라한다.
- 모듈은 특징에 따라 여러유형으로 분리할 수 있으며 그들이 다루는 관심사나 기능에 따라 구조가 달라진다.
- 최상위의 분류로 수직적 관심사 ,수평적 관심사로 나뉜다
- 모듈 설계시 가장 중요한 것은 두가지 관심사를 섞지 않는 것이다.

| -    | 수직적 관심사                                          | 수평적 관심사                                                       |
|------|--------------------------------------------------|---------------------------------------------------------------|
| 특징   | 비지니스와 응용프로그램에 특화된 기능을 제공하고 비지니스 도메인의 특정문제를 해결한다. | 비지니스나 응용프그램에 특화되지 않은 일반적인 기능으로  , 공통적인 저수준으 기능 혹은 프레임워크를 말한다. |
| 우선사항 | 비지니스 도메인의 요구사항을 효과적으로 처리하도록 설계하는 것이 가장 우선 된다.    | 범용적으로 여러곳에서 사용될 수 있어야 하기 때문에 재사용성이 매우 중요하다                    |



#### 스프링 애노테이션 기반 캐싱

- 스프링 AOP 기반으로 동작하며 캐싱을 위한 5가지 애노테이션을 제공한다.
- 선언적 캐싱을 위해선 빈 구성정보 클래스에서 빈 설정 정보 애노테이션인 @EnableCaching 선언이 필요하다.
- CachingConfigurer를 구현해 @EnableCaching에 대해 더 세밀하게 설정할 수 있다.
...
- 지역화는 소프트웨어를 각 환경에 대해 지원하는 것을 의미한다.
- 언어 외에도 문자열 출력 형식 ,화폐 , 날짜 ,측정 단위 타임존 , UI , Os/Platform 등 다양한 요소를 고려해야한다.
- 국제화를 약어로 i18n 이라하고 지역화를 약어로 l10n 이라 부른다.
...
---
> #### 우리는 오늘 완성해야 하는 기능을 구현하는 코드를 짜야하는 동시에 내일 쉽게 변경할 수 있는  코드를 짜야 한다 -Sandi Metz-

> #### 객체지향 설계란 올바른 객체에게 올바른 책임을 할당하면서 낮은 결합도와 높은 응집도를 가진 구조를 창조하는 활동이다 -Marc Evers, Rob WestGeest-

- 객체지향의 핵심은 각 객체의 책임이다.
- 책임을 할당하는 작업이 응집도와 결합도와 같은 설계 품질과 깊이 연관되어 있다.
- 설계는 변경을 위해 존재하고 변경에는 어떤식으로든 비용이 발생한다.
- 훌륭한 설계란 합리적인 비용안에서 변경을 수용할 수 있는 구조를 만드는 것이다.

#### 복잡함을 다루기 위한 스프링의 전략

- 비지니스 로직을 담은 애플리케이션 핵심 코드와 엔터프라이즈 기술을 처리하는 코드를 분리시킨다.
- 각각의 복잡함을 효과적으로 공략하기 위해 POJO와 함께 스프링 3대 핵심 기술을 사용한다.
- 이 전략이 효과적으로 적용되려면 받듯이 좋은 객체지향 설계가 바탕 되어야 한다.
